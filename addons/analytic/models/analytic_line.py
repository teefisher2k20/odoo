...
class AnalyticPlanFields(models.AbstractModel):
    ...
    # Magic column that represents all the plans at the same time, except for
    # the compute where it is context dependent, and needs the id of the
    # desired plan.
    # Used as a syntactic sugar for search views, and magic field for
    # one2many relation
    ...
    def _compute_auto_account(self):
        plan_id = self.env.context.get('analytic_plan_id')
    ...
    def _inverse_auto_account(self):
        for line in self:
            column_name = line.auto_account_id.plan_id._column_name()
            line[column_name] = line.auto_account_id

    def _search_auto_account(self, operator, value):
        all_plans = self.env['account.analytic.plan']._get_all_plans()
        return OR([
            [(plan._column_name(), operator, value)]
            for plan in all_plans[0] + all_plans[1]
        ])
    ...
    def _get_plan_fnames(self):
        ...
        return [fname for fname in fnames if fname in self]
    ...
    def _get_distribution_key(self):
        ...
        return ",".join(str(account_id) for account_id in account_ids)
    ...
    def _get_mandatory_plans(self, company, business_domain):
        ...
        relevant_plans = plans.sudo().with_company(company).get_relevant_plans(
            business_domain=business_domain, company_id=company.id)
        return [
            {'name': p['name'], 'column_name': p['column_name']}
            for p in relevant_plans if p['applicability'] == 'mandatory'
        ]
        ...
        for line in self:
            if not any(line[fname] for fname in fnames):
                raise ValidationError(
                    _("At least one analytic account must be set"))

    @api.model
    def fields_get(self, allfields=None, attributes=None):
        fields = super().fields_get(allfields, attributes)
        has_access = self.env['account.analytic.plan'].has_access('read')
        if not self._context.get("studio") and has_access:
            all_plans = self.env['account.analytic.plan']._get_all_plans()
            for plan in all_plans[0] + all_plans[1]:
    ...
    def _patch_view(self, arch, view, view_type):
        ...
        if not self._context.get("studio") and has_access:
            ...
            # Force domain on main account node, fields_get doesn't do the trick
            if account_node is not None and view_type == 'search':
                domain = self._get_plan_domain(project_plan)
                account_node.set('domain', repr(domain))

            # If there is a main node, append the ones for other plans
            if account_node is not None:
                context = self._get_account_node_context(project_plan)
                account_node.set('context', repr(context))
            ...
            if account_filter_node is not None:
                for plan in other_plans[::-1] + project_plan:
                    ...
                    if plan != project_plan:
                        context = f"{{'group_by': '{fname}'}}"
                        account_filter_node.addnext(E.filter(
                            name=fname, context=context
                        ))
                    current = plan
...
class AccountAnalyticLine(models.Model):
    ...
    def _inverse_analytic_distribution(self):
        ...
        to_create_vals = []
        for line in self:
            merged_dist = self.env['analytic.mixin']._merge_distribution
            final_distribution = merged_dist(
            ...
            def _get_accounts(account_ids):
                ids = [int(aid) for aid in account_ids.split(',')]
            ...
            vals_list = [
                ...
                | empty_account | {
                    ...
                    for account in _get_accounts(account_ids)
                }
            ...
            to_create_vals += [
                line.copy_data(vals)[0] for vals in vals_list[1:]
            ]
        if to_create_vals:
    ...
    def _condition_to_sql(self, alias, fname, operator, value, query):
        if fname == 'date' and value == 'fiscal_start_year':
            ...
            value = fiscalyear_dates['date_from'] - relativedelta(years=1)
