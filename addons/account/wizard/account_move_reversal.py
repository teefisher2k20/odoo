...
class AccountMoveReversal(models.TransientModel):
    ...
    move_ids = fields.Many2many('account.move', 'account_move_reversal_move',
                                'reversal_id', 'move_id', domain=[('state', '=', 'posted')])
    ...
    currency_id = fields.Many2one(
        'res.currency', compute="_compute_from_moves")
    ...
    def _compute_journal_id(self):
        for record in self:
            if record.journal_id:
                ...
                journals = record.move_ids.journal_id.filtered(
                    lambda x: x.active)
    ...
    def _compute_from_moves(self):
        for record in self:
            ...
            record.move_type = move_ids.move_type if len(move_ids) == 1 else (any(move.move_type in (
                'in_invoice', 'out_invoice') for move in move_ids) and 'some_invoice' or False)
    ...
    def reverse_moves(self, is_modify=False):
        ...
        for moves, default_values_list, is_cancel_needed in batches:
            ...
            moves._message_log_batch(
                bodies={move.id: move.env._('This entry has been %s', reverse._get_html_link(
                    title=move.env._("reversed"))) for move, reverse in zip(moves, new_moves)}
            )
            ...
            if is_modify:
                ...
                for move in moves.with_context(include_business_fields=True):
                    data = move.copy_data(
                        self._modify_default_reverse_values(move))[0]
                    data['line_ids'] = [line for line in data['line_ids'] if line[2]
