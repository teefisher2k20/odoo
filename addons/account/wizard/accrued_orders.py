...
class AccruedExpenseRevenue(models.TransientModel):
    ...
    def _compute_display_amount(self):
        ...
        for record in self:
            ...
            lines = preview_data.get('groups_vals', [])[
                                     0].get('items_vals', [])
            record.display_amount = record.amount or (
                single_order and not lines)
    ...
    def _compute_preview_data(self):
        for record in self:
            ...
            preview_columns = [
                ...
                {'field': 'debit', 'label': _(
                    'Debit'), 'class': 'text-end text-nowrap'},
                {'field': 'credit', 'label': _(
                    'Credit'), 'class': 'text-end text-nowrap'},
            ]
    ...
    def _compute_move_vals(self):
        ...
        for order in orders:
            ...
            if len(orders) == 1 and product_lines and self.amount and order.order_line:
                ...
                values = _get_aml_vals(order, self.amount, 0, account.id, label=_(
                    'Manual entry'), analytic_distribution=distribution)
                ...
                if is_purchase:
                    ...
                    o.order_line.with_context(
                        accrual_entry_date=self.date)._compute_qty_invoiced()
                    o.order_line.with_context(
                        accrual_entry_date=self.date)._compute_untaxed_amount_invoiced()
                    o.order_line.with_context(
                        accrual_entry_date=self.date)._compute_qty_to_invoice()
                ...
                for order_line in lines:
                    if is_purchase:
                        ...
                        amount = order.currency_id._convert(
                            amount_currency, self.company_id.currency_id, self.company_id)
                        fnames = ['qty_to_invoice', 'qty_received',
                                  'qty_invoiced', 'invoice_lines']
                        label = _(
                            ...
                            unit_price=formatLang(
                                self.env, amount_currency / order_line.qty_to_invoice, currency_obj=order.currency_id),
                        )
                    else:
                        account = self._get_computed_account(
                            order, order_line.product_id, is_purchase)
                        ...
                        amount = order.currency_id._convert(
                            amount_currency, self.company_id.currency_id, self.company_id)
                        fnames = ['qty_to_invoice', 'untaxed_amount_to_invoice',
                    ...
                    values = _get_aml_vals(
                        order, amount, amount_currency, account.id, label=label, analytic_distribution=distribution)
                    move_lines.append(Command.create(values))
        ...
        if not self.company_id.currency_id.is_zero(total_balance):
            ...
            values = _get_aml_vals(orders, -total_balance, 0.0, self.account_id.id, label=_(
                'Accrued total'), analytic_distribution=analytic_distribution)
            move_lines.append(Command.create(values))
    ...
    def create_entries(self):
        ...
        for order in orders_with_entries:
            order.message_post(
                body=self._get_accrual_message_body(move, reverse_move))
        return {
